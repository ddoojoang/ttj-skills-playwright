...
`ttj-skills-browser crawl`은 두 종류의 크롤링 대상을 함께 보여준다:
- 🔴 **레이아웃 영역** (`type: section`, 빨간 박스) — 페이지 구역 단위 (헤더/사이드바/메인 등)
- 🔵 **반복 목록** (`type: list`, 파란 박스 🕷) — 같은 구조가 3개 이상 반복되는 컨테이너
  (상품 카드, 기사 목록, 리뷰 행 등 — **크롤링 가치가 가장 높은 부분**)

동작:
- 각 영역에 배지(`e1 🕷 ×6` = 반복 항목 6개)와 굵은 아웃라인 표시
- **배지에 마우스를 올리면 그 박스만 남고 나머지는 모두 숨겨지며** 셀렉터 라벨이 표시됨
- 배지 클릭 시 컨테이너 셀렉터 클립보드 복사
- 분석 결과 JSON을 stdout에 출력 (진행 로그는 stderr — stdout만 파싱하면 순수 JSON).
  각 항목에 `type`, `count`, `crawlScore`(가치 점수, 높은 순 정렬), `fields`(links/images/hasPrice/hasDate), `sample` 포함
- 전체 페이지 스크린샷을 임시 폴더에 저장 (경로는 로그의 "📸 스크린샷 저장:" 줄)

**AI 실행 절차:**
1. `ttj-skills-browser crawl` 실행
2. 로그의 "📸 스크린샷 저장:" 경로를 Read 도구로 사용자에게 표시
3. JSON을 `crawlScore` 높은 순으로 리스트업 (type·sample로 무엇인지 해석):

| Ref | 유형 | 설명 (샘플 기반) | 항목 수 | 추출 가능 필드 | 컨테이너 셀렉터 |
|-----|------|------------------|--------|---------------|----------------|
| e1 | 🔵목록 | 호텔 카드 목록 | 30 | 제목, 링크, 이미지, 가격 | `div#hotel-list` |
| e2 | 🔵목록 | 리뷰 목록 | 12 | 텍스트, 날짜 | `ul.reviews` |
| e3 | 🔴영역 | 사이드바 위젯 | 5 | 링크 | `aside.rank` |

4. **AI가 크롤링 가치를 꼼꼼히 판단해 먼저 제안한다** — `crawlScore`를 그대로 읽지 말고,
   아래 기준으로 각 후보를 직접 검토한 뒤 추천한다:

   **✅ 진짜 데이터 (추천 대상)** — 항목마다 여러 필드가 규칙적으로 반복:
   - `sample`에 제목+부가정보(날짜/출처/가격/설명)가 함께 보임
   - `fields.avgItemChars`가 큼(≈30자 이상), `images`나 `hasDate`/`hasPrice`가 있음
   - 예: 기사 목록, 상품 카드, 리뷰, 검색 결과, 게시글 목록

   **❌ 데이터 아님 (제외하거나 후순위)** — 반복되지만 값이 없음:
   - `looksLikeNav: true` 또는 `avgItemChars`가 매우 작음(<12) → **네비게이션/메뉴/탭** (링크 텍스트만)
   - 페이지네이션, 카테고리 칩, 소셜 버튼, 광고 배너
   - `type: section`(🔴 레이아웃) 은 데이터가 아니라 구역 — 그 안의 🔵 목록을 봐야 함

   판단 후 **추천 + 이유 + 무엇을 뽑을 수 있는지**를 붙여 제안한다. 예:
   > "이 페이지에서 수집 가치가 높은 건 **e1 기사 목록(20건 — 제목·언론사·시간)**과
   >  **e3 랭킹 뉴스(10건 — 제목·조회수)**입니다. e2는 카테고리 메뉴라 데이터가 아니라 제외했습니다.
   >  어떤 걸 크롤링할까요?"

   애매하면 후보 셀렉터로 `eval`을 한 번 돌려 **실제 항목 텍스트를 확인한 뒤** 판단한다.

5. 사용자가 `e1` 같은 ref나 항목을 지정하면, 해당 컨테이너 셀렉터 기준으로 `eval`로
...
**개발 절차 — 반드시 아래 3단계를 순서대로. 어느 하나도 건너뛸 수 없다.**

**① DOM 검증 (코드 작성 전)**
1. `eval`로 열린 탭에서 셀렉터·추출 로직을 먼저 테스트:
   ```bash
   ttj-skills-browser eval "(() => [...document.querySelectorAll('.item')].map(el => el.textContent))()"
   ```
2. 예상 데이터가 나오는지, 누락·중복·빈 값이 없는지 확인
3. 🚫 **검증되지 않은 셀렉터로 코드를 작성하지 않는다** (DOM 검증 생략 금지)

**② 코드 작성**
4. ①에서 **실제로 동작이 확인된 셀렉터·로직만** 사용해 함수 단위로 작성

**③ 실행 테스트 (코드 작성 후)**
5. 작성한 코드를 **AI가 직접 실행**한다 (headless: false, 단계별 로그 출력)
6. 오류·빈 결과·셀렉터 미스가 나오면 수정 → 재실행을 **정상 동작할 때까지 반복**
7. 🚫 **실행 테스트로 결과를 확인하기 전에는 "완료"라고 보고하지 않는다.**
   최종 보고에는 반드시 **실행 결과(수집 건수 + 샘플 데이터)**를 포함한다.

> ⚠️ 요약 강제 규칙: **DOM 검증 없이 코드 작성 금지 · 실행 테스트 없이 완료 보고 금지.**
> 이 둘은 크롤링/자동화 코드 산출물의 필수 관문이다.
